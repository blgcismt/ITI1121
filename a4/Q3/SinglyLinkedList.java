...
public class SinglyLinkedList<E> implements List<E>, Iterable<E> {
    ...
    public int count(E fromElement, E toElement) {
        return count(head, fromElement, toElement, 0); // call to the recursive count method
    }

    private int count(Node<E> p, E fromElement, E toElement, int count) {

        if (p == null) { // end of the list reached
            return count; // return the count
        }
        
        else if (count == 0){ // if the count is 0, else if is used because we only want to increment the count once when the first element is found

            if (p.value.equals(fromElement)){ // if the value is equal to the fromElement 
                count = 1; // increment the count
            }

            if (p.value.equals(toElement) && count == 1){ // if the value is equal to the toElement
                return count; // return the count
            }

        }

        else if (count > 0){ // if the count is greater than 0 
            count++; // increment the count because we are counting the distance between the two elements
            if (p.value.equals(toElement)){ // if the value is equal to the toElement
                return count; // return the count
            }
        }

       return count(p.next, fromElement, toElement, count); // recursive call to count
    } 
